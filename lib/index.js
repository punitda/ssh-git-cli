...
if (program.generate) {
  inquirer
    ...
    .catch(error => {
      log(
        chalk.blue(
          'Looks like we messed up somewhere. :(. Please help us by copying the error posted below and creating a github issue for it using the link provided below. \n'
        ) +
          chalk.red(`${error}\n`) +
          chalk.blue.bold(`${manifest.bugs.url}`)
      );
    });
} else {
  log(
