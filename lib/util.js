...
const getMessages = () => {
  if (process.platform === 'darwin') {
    return [
      `> Generating ssh keys...`,
      `> Starting ssh-agent...`,
      `> Adding generated ssh keys to ssh-agent and to the OSX keychain so that you wouldn't be prompted for passpharse of ssh keys again when using ssh-agent after system restarts.\nPlease enter the passphrase you just entered in the previous step to add it to keychain`,
    ...
    return [
      `> Generating ssh keys...`,
      `> Starting ssh-agent...`,
      `> Adding generated ssh keys to the ssh-agent.\nPlease enter the passphrase you just entered in the previous step to add generated key to ssh-agent.`,
...
const showNextSteps = config => {
  if (process.platform === 'darwin') {
    ...
    log(
      chalk`{green.bold SSH key and config generated successfully 🎉  and public key has been printed to the console(check output above). Please copy it to your Clipboard. \n\n}`
    );
    log(
      chalk`{blue.bold To start using this ssh key, you need to follow below few steps :\n1. Login into your ${
        config.hostingProvider
